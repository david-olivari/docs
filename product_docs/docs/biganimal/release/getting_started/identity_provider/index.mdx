...
If you purchased through Azure Marketplace, BigAnimal authenticates users using Azure Active Directory (AD) and you don't have to complete these steps. Azure AD is linked during subscription.
...
A verified domain is necessary for your users to have a streamlined login experience. 

In the **Domains** tab, enter the domain and select **Next: Verify Domain**. Copy the TXT record and follow the instructions. Select **Done**. 

Your domain and its status appears on the **Domains** tab, where you can delete or verify it. To add another domain, select **Add Domain**. 

When you have at least one verified domain, the identity provider status becomes **Active** on the the **Identity Providers** tab. 

!!! Note
    The identity provider status can take up to three minutes to update after verifying a domain. 
...
After setting up your cloud server provider, you can assign roles to your default project. See [Managing portal access](/biganimal/release/administering_cluster/01_portal_access).
